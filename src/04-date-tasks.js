...
function timeSpanToString(startDate, endDate) {
  ...
  const ms = Math.floor((((time / 1000 / 60 / 60 - h) * 60 - m) * 60 - s) * 1000);
  return `${h.toString().padStart(2, '0')}:${m.toString()
    .padStart(2, '0')}:${s.toString().padStart(2, '0')}.${ms.toString().padStart(3, '0')}`;
}
...
function angleBetweenClockHands(date) {
  let h = (date.getHours() - 3 + 24) % 24;
  if (h > 12) h -= 12;
  const m = date.getMinutes();
  let result = Math.abs((((h + (m / 60)) * 30 - m * 6) * Math.PI) / 180);
